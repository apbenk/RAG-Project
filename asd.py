...
ai_client = OpenAI(api_key="AIzaSyB6V3ajv...", 
...
# Ubah bagian fungsi process_pdf ini di dalam app.py
...
def process_pdf(file, user_id):
    ...
    if len(text) < CHUNK_SIZE:
        chunks = [text] # Jika teks pendek, jadikan 1 chunk
    else:
    ...
    # Simpan chunks dengan separator
    ...
    with db_cursor(commit=True) as cur:
        cur.execute("INSERT INTO documents (filename, pdf_path, faiss_path, user_id) VALUES (%s, %s, %s, %s)",
                    (fname, pdf_path, faiss_path, user_id))
...
def ask():
    ...
    try:
        ...
        with db_cursor() as cur:
            ...
            cur.execute(f"SELECT filename, faiss_path FROM documents WHERE id IN ({format_strings})", tuple(doc_ids))
            docs = cur.fetchall()
        ...
        for doc in docs:
            try:
                ...
                for score, idx in zip(D[0], I[0]):
                    if idx < len(chunks):
                        candidates.append({"score": score, "text": chunks[idx], "source": doc['filename']})
            except: continue
